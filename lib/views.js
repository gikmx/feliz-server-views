...
function onReady(server){
    ...
    else this.conf = this.util.object(Conf).merge({ views: this.conf.views });
    ...
    if (!this.util.is(conf.index).string()) throw error.type({
        name: `${Name}.index`,
        type: 'String',
        data: conf.index
    });
...
function onRequest(request, reply){
    ...
    this.server._replier._decorations.view = function(){
        ...
        args.unshift(PATH.join(route.root, this.path.views.index))
        return renderer.apply(this, args);
    }
    reply.continue();
}
